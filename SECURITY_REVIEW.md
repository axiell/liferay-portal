...
**Date:** 2026-09-07
**Source scan:** `aikido-scan --full --no-fail --force-create-repository-for-branch` (all six stages completed: code, secrets, dependencies, SAST, licenses, IaC; completed in ~45m after activating the branch-specific Aikido repo row)
**Dashboard:** https://app.aikido.dev/repositories/2914481 (repo `axiell/liferay-portal`, branch `arena-7.4.3.129-ga129`, head `033c6fa`)
...
|---|---|---|
| Latest clean scan (this tree, current excludes) | **57** | What the scanner reports on the checked-out working tree today |
...
The 2026-09-07 rerun completed cleanly at the scanner level and reported **57** open issues in the scan summary, versus **55** on 2026-08-29. Treat that delta as normal dashboard drift between runs, not a change in remediation priority: the same buckets still dominate.
...
**Action:** file-by-file verdict — "test fixture / not shipped" (ignore with justification + review date) or rotate-and-remove. Start with `.idea/workspace.xml`, any shipped `portlet.properties` or runtime config files, and anything under non-test resource trees. For fixtures kept in-tree, replace realistic-looking values with obvious placeholders where possible before choosing the ignore route.

---

## 5. IaC, EOL, license (in-tree)

- **IaC:** 84 open repo-wide, all container hygiene (Docker runs as root: 52+1; auto-upgrading base images: 30; S3 public-access block: 1). Small, fixable in Dockerfiles by running as a non-root user, pinning image tags/digests instead of auto-updating bases, and ensuring S3 public-access blocking stays explicit where that IaC applies.
...
1. **Secrets review (§4)** — 30 minutes of eyes; rotate anything real. Highest signal-per-effort. Suggested method: clear `.idea/workspace.xml` first, then shipped resource/config files, then remaining test fixtures.
2. **axios 1.6.2 → 1.6.x patch (group 21258589)** — 73 records, one dependency family, one PR. Verify the consuming manifests/lockfiles first; fix the resolved version at the real source, not generated output.
3. **Frontend dependency families, one PR each:** lodash 4.17.21, lodash-es 4.17.11, billboard.js 1.12.11, dompurify 3.0.2, follow-redirects 1.15.3, @babel/traverse 7.16.10, rollup 1.32.1, tar 7.2.0. For each one: update the manifest or lockfile that actually pins it, regenerate only what the build normally regenerates, then rescan.
4. **bcprov-jdk18on 1.78 → 1.78.x** (group 42882240). Confirm the exact consumer module first so one version change clears the whole group.
5. **Platform-pinned candidates — do NOT bump yet; check the platform pin first:** Spring Framework 5.3.39 (Liferay 7.4 bundles the 5.3.x line), Spring Boot 2.7.18, Spring Security 5.5.1, spring-core/spring-expression 4.1.9. If these versions are inherited from Liferay/platform BOMs or bundled components, record them in `.aikido-ignore` via the bulk-ignore flow (§7) with justification, owner, and review date rather than forcing unsupported upgrades.
6. **IaC Dockerfile fixes** — runs-as-root and base-image pinning; ~84 findings, mostly mechanical. Prioritize shared Dockerfile templates or common base definitions first so one edit removes several findings.
7. **License policy decision** for the AGPL/SSPL/Elastic set. Capture the business decision explicitly; this is not a technical CVE-style bump.
8. **Contamination cleanup (§3)** — admin-level, separate from this branch's remediation. Best fix is repo separation or naming correction in Aikido so future exports stop mixing foreign trees into this branch's backlog.
